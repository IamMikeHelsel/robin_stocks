...
def login_first_time(
    encryption_passcode, client_id, authorization_token, refresh_token
):
    ...
    with pickle_path.open("wb") as pickle_file:
        pickle.dump(
            {
                "authorization_token": cipher_suite.encrypt(
...
def login(encryption_passcode):
    ...
    pickle_path = data_dir.joinpath(PICKLE_NAME)
    if not pickle_path.exists():
        raise FileExistsError("Please Call login_first_time() to create pickle file.")
