...
def post_symbols_to_watchlist(inputSymbols, name="My First List"):
    ...
    for id in ids:
        payload = {
            watchlist_id: [
                {"object_type": "instrument", "object_id": id, "operation": "create"}
            ]
        }
...
def delete_symbols_from_watchlist(inputSymbols, name="My First List"):
    ...
    for id in ids:
        payload = {
            watchlist_id: [
                {"object_type": "instrument", "object_id": id, "operation": "delete"}
            ]
        }
        url = watchlists_url(name, True)
        data.append(request_post(url, payload, json=True))
    ...
    for item in positions_data:
        ...
        try:
            ...
            holdings[symbol].update(
                {"intraday_percent_change": f"{intraday_percent_change:.2f}"}
            )
...
def build_user_profile(account_number=None):
    ...
    if accounts_data:
        cash = "{:.2f}".format(
            float(accounts_data["portfolio_cash"])
